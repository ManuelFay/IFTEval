...
regex = re.compile("e2_(bloom_|llama_|falcon_|pythia_)[a-zA-Z0-9]+_\d+_\d_predictions.jsonl")
...
for file in tqdm(l1):
    ...
    r["task"] = file.split("_")[2]
    r["num_samples"] = file.split("_")[3]  # .split(".")[0] --> for old format
    r["run_id"] = file.split("_")[4]
    r["file"] = file
    results.append(r)

out_name = "outputs/results_exp1b_standard_datasets_all.csv"
if os.path.exists(out_name):
    import datetime

    out_name = f"outputs/results_exp1b_standard_datasets_all_{datetime.date.today()}.csv"
