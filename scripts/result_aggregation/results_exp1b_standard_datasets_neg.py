...
datasets = ["mnli", "sst2", "conll"]
# regex = re.compile('fractial_[a-zA-Z0-9]+_\d+_predictions.jsonl')
regex = re.compile("e2_(bloom_|llama_|falcon_|pythia_)[a-zA-Z0-9]+_(1000|0)_0_synth_predictions.jsonl")
LM_SAMPLE_SCORING = 100

# BASE_DIR = "outputs/results/results_2804/"
BASE_DIR = "outputs/results/results_new/"
results = []
l1 = os.listdir(BASE_DIR)
l1 = sorted([x for x in l1 if regex.match(x) and x.split("_")[2] in datasets])
...
for file in tqdm(l1):
    ...
    print(file)
    # print(file, r["custom_score"], r["bertscore"], r["sbertscore"], r["reward_model_score"], r["gpt-4_lm_score"], r["gpt-3.5-turbo_lm_score"])
    r["model"] = file.split("_")[1]
    r["negtask"] = file.split("_")[2]
    r["num_samples"] = file.split("_")[3]  # .split(".")[0] --> for old format
    r["run_id"] = file.split("_")[4]
    r["file"] = file
    results.append(r)

out_name = "outputs/results_exp1b_standard_datasets_neg.csv"
if os.path.exists(out_name):
    import datetime

    out_name = f"outputs/results_exp1b_standard_datasets_neg_{datetime.date.today()}.csv"
